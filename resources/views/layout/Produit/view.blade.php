        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg px-3">
            <div class="flex items-center justify-center mb-5 ">
                <div class=" w-full p-3 ">
                    <div class=" px-4 ">
                        <h1 class="text-2xl font-bold antialiased pb-3 pt-6 text-green-600">Information sur le produit
                        </h1>
                    </div>
                    <div class="p-6 bg-white border border-gray-200 rounded-lg shadowdark:bg-gray-800 ">
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Nom : {{ $produits->nom_p }}
                        </h5>
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Libelle :
                            {{ $produits->libelle_p }} </h5>
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Reference :
                            {{ $produits->ref_p }} </h5>
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Quantité :
                            {{ $produits->qte_p }} </h5>
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Quantité Alert :
                            {{ $produits->qte_alert }} </h5>
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Date D'Entrée :
                            {{ $produits->date_enter }} </h5>
                    </div>
                    <div class="px-6 py-3 flex justify-end">
                        <a href="{{ url('/produits/' . $produits->id_produit . '/edit') }}" title="Edit Product">
                            <button type="button"
                        ...
                        <form method="post" action=" {{ url('/produits/' . $produits->id_produit) }} ">
                            @csrf
                            {{ method_field('DELETE') }}
                            <button type="submit" title="Delete Student" onclick=" return confirm('confirm delete ? ') "
                                type="button"
